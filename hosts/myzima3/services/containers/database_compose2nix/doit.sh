...
if [ $# -ne 2 ]
then
  echo "usage: $0 <docker-compose.yml filename not path> <nixFile.nix>"
  exit 13
fi
...
args="${args} -output $2"                           # path to output Nix file. (default "docker-compose.nix")
...
args="${args} -runtime docker"                      # one of: ["podman", "docker"]. (default "podman")
